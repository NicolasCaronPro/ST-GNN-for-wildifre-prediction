...
if __name__ == '__main__':
    ...
    for dep in trainDepartements:
        ...
        tar = read_object(dep+'influence.pkl', path_to_target)[:,:,:trainMax]
    ...
    save_object(encoder, 'encoder_calendar.pkl', Path('Encoder'))

    # Landcover
    encoder = CatBoostEncoder(cols=np.arange(0, 1))
    encoder.fit(landcover, gt)
    save_object(encoder, 'encoder_landcover.pkl', Path('Encoder'))

    # Foret
    encoder = CatBoostEncoder(cols=np.arange(0, 1))
    encoder.fit(foret, gt)
    save_object(encoder, 'encoder_foret.pkl', Path('Encoder'))

    # Geo
    encoder = CatBoostEncoder(cols=np.arange(0, 1))
    encoder.fit(geo_array, gt)
    save_object(encoder, 'encoder_geo.pkl', Path('Encoder'))