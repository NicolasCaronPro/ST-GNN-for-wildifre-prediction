...
def get_sub_nodes_ground_truth(graph, subNode: np.array,
                           departements : list,
                           oriNode : np.array, path) -> np.array:
        ...
        for departement in departements:
            ...
            if graph.train_kmeans:
                dir_mask = path / 'raster' / '2x2'
                dir_bin = path / 'bin' / '2x2'
                dir_proba = root / 'Model' / 'HexagonalScale' / 'log'
                dir_model = root / 'Model' / 'HexagonalScale' / 'logisticRegression'
                dir_predictor = root /  'Model' / 'HexagonalScale' / 'GNN' / 'influenceClustering'
            else:                                                                                                           #
                dir_mask = Path('/home/caron/Bureau/Model/HexagonalScale/raster')                                           #
                dir_proba = Path('/home/caron/Bureau/Model/HexagonalScale/proba')                                           #   <-------------- Not really use but we never know
                dir_model = Path('/home/caron/Bureau/Model/HexagonalScale/logisticRegression')                              #
                dir_predictor = Path('/home/caron/Bureau/Model/HexagonalScale/GNN/influenceClustering')                     #
                dir_bin = Path('/home/caron/Bureau/Model/HexagonalScale/bin')                                               #
            
            modelWeight = read_object(departement+'Predictor.pkl', dir_predictor)

            mask = read_object(departement+'rasterScale'+str(graph.scale)+'.pkl', dir_mask)
            array = read_object(departement+'Influence'+'.pkl', dir_proba)
            ...
            for node in nodeDepartement:
                ...
                if array[:, :, int(node[4] + 1)][maskNode].shape[0] != 0:
                    ...
                    # Sample weight
                    if indexOri.shape[0] > 0:
                        w = modelWeight.predict(np.array(np.max(array[:, :, int(node[4] + 1)][maskNode])))
                        Y[index, -3] = modelWeight.weight(w)[0]
                    else:
                        Y[index, -3] = 0

                    # Influence
                    values = np.nanmax(array[:, :, int(node[4] + 1)][maskNode])
                    #values = maxInfluence - np.nanprod(np.unique(array[:, :, int(node[4] + 1)]))
                    Y[index, -1] = round(values, 3)

                    # Nb events
                    values = np.sum(np.unique(arrayBin[:, :, int(node[4] + 1)][maskNode]))
                    Y[index, -2] = values
...
def get_sub_nodes_feature(graph, subNode: np.array,
    ...
    dir_encoder = root / 'Model/HexagonalScale/GNN/Encoder'
    ...
    for departement in departements:
        ...
        if graph.train_kmeans:
            ...
            dir_mask = root / 'Model/HexagonalScale/raster'
