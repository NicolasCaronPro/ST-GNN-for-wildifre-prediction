...
scaleTrain = int(sys.argv[2])
readGraph = sys.argv[3] == 'read'
doDatabase = sys.argv[4] == 'doDatabase'
do2D = sys.argv[5] == 'do2D'
newFeatures = []

############### Global variable ###############
train_dir = Path('train')
prefix_train = str(minPoint)+'_'+str(k_days)+'_'+str(scaleTrain)#str(scale)
if dummy:
    prefix_train = str(k_days)+'_'+str(scale)+'_dummy'
...
def test(testname, testDate, pss, geo, testDepartement, dir_output):
    ...
    if len(newFeatures) > 0:
        print(X.shape)
        subnode = X[:,:6]
        XnewFeatures, _ = get_sub_nodes_feature(graphScale, subnode, departements, newFeatures, dir_output)
    ...
    Xset, Yset = preprocess_test(X=X, Y=Y, Xtrain=Xtrain, Ytrain=Ytrain, pos_feature=pos_feature, scaling=scaling) 
    ...
    metrics['GT'] = add_metrics(methods, 0, Ypred, YTensor, testDepartement, False, scale, 'gt')
    ...
    for mddel, use_temporal_as_edges, isBin, is_2D_model  in models:
        ...
        if not is_2D_model:
            test_loader = load_loader_test(use_temporal_as_edges=use_temporal_as_edges, graphScale=graphScale, dir_output=dir_output,
                                        ...
                                        scaling=scaling, encoding=encoding, prefix=prefix)
        ...
        graphScale._load_model_from_path(Path('check_'+scaling+'/' + prefix_train + '/' + mddel +  '/' + 'best.pt'),
                                        dico_model[mddel], device)

        Ypred, YTensor = graphScale._predict_test_loader(test_loader)

        metrics[mddel] = add_metrics(methods, i, Ypred, YTensor, testDepartement, isBin, scale, mddel)

        pred = Ypred.detach().cpu().numpy()
        y = YTensor.detach().cpu().numpy()

        realVspredict(pred, y,
                      dir_output, mddel+'_'+prefix_train+'_'+str(scale)+'_'+scaling+'_'+encoding+'_'+testname+'.png')
        ...
        realVspredict2d(Ypred.detach().cpu().numpy(), YTensor.detach().cpu().numpy(),
                      dir_output / n)"""
    ...
    for name, isBin in tradiModels:
        ...
        print(f'      {name}            ')
        print('#########################')
        XTensor, YTensor, _ = load_tensor_test(use_temporal_as_edges=True, graphScale=graphScale, dir_output=dir_output,
                                                        X=X, Y=Y, Xtrain=Xtrain, Ytrain=Ytrain, device=device, k_days=k_days,
                                                        test=testname, pos_feature=pos_feature,
                                                        scaling=scaling, prefix=prefix, encoding=encoding)
        ITensor = YTensor[:,-3]
        model = read_object(name+'.pkl', Path('check_'+scaling+'/' + prefix_train + '/baseline/' + name + '/'))
        graphScale._set_model(model)
        Ypred = graphScale.predict_model_api_sklearn(XTensor.detach().cpu().numpy(), isBin)

        Ypred = torch.tensor(Ypred, dtype=torch.float32, device=device)
        YTensor = YTensor[ITensor.gt(0)]
        Ypred = torch.masked_select(Ypred, ITensor.gt(0))

        metrics[name] = add_metrics(methods, i, Ypred, YTensor, testDepartement, isBin, scale, name)
        ...
        realVspredict2d(Ypred.detach().cpu().numpy(), YTensor.detach().cpu().numpy(), dir_output /  n)"""
...
test('dummy', None, dummyGeo, geo, testDepartement, Path('test/'))

# 2023 test
test('2023', testDates, geo, geo, testDepartement, Path('test/'))

# 2023 test Ain
testDepartement = ['departement-01-ain']
test('Ain', testDates, geo, geo, testDepartement, Path('test/'))

# 2023 test Doubs
testDepartement = ['departement-25-doubs']
test('Doubs', testDates, geo, geo, testDepartement, Path('test/'))

# 2023 test Yvelines
testDepartement = ['departement-78-yvelines']
test('Yvelines', testDates, geo, geo, testDepartement, Path('test/'))

# 69 test
testDates = find_dates_between('2018-01-01', '2023-01-01')
testDepartement = ['departement-69-rhone']

test('69', testDates, geo, geo, testDepartement, Path('test/'))