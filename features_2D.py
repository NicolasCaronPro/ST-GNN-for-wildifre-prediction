...
def get_sub_nodes_feature_2D(graph, shape: int,
    ...
    for departement in departements:
        ...
        for i, node in enumerate(nodeDepartement):
            ...
            # Add landcover
            if doLand:
                #raster_landcover(spatioTemporalRaster, allDates[indD], node[0], mask,
                #                 rootDisk / departement / 'data', pos_feature['landcover'], encoder_landcover)
                raster_landcover_2(spatioTemporalRaster, node[0], mask, landcover, pos_feature['landcover'], encoder_landcover)
            ############################################################################################################################
            
            # Add foret
            if doForet:
                raster_foret(spatioTemporalRaster, mask, node[0], elevation, pos_feature['foret'], encoder_foret)
