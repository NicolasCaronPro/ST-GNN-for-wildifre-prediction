...
class Training():
    ...
    def calculate_prox_term(self, names):
        prox = 0.0
        for (name, p), (_, pg) in zip(model.named_parameters(), global_model.named_parameters()):
    ...
    def get_class_freq(self, df_train):
        ...
        for i, cl in enumerate(uclass):
            res[i] = len(df_train[df_train[self.target_name] == cl])
...
class Model_susceptibility():
    def __init__(self, model_name, target, resolution, model_config, features_name, out_channels, task_type, ks, departements, train_departements, train_date, val_date, dir_log, horizon=0):
            self.model_name = model_name
