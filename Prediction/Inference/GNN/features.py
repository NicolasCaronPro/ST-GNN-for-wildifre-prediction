    ...
    if 'AutoRegressionReg' in features:
        for node in subNode:
            ...
            for var in auto_regression_variable_reg:
                ...
                save_value(geo[f'AutoRegressionReg'].values, f'AutoRegressionReg-{var}', index, maskNode)
    ...
    if 'AutoRegressionBin' in features:
        for node in subNode:
            ...
            for var in auto_regression_variable_bin:
                step = int(var.split('-')[-1])
                maskNode = geo[(geo['id'] == node[id_index]) & (geo['date'] == node[date_index] - step)].index
                save_value(geo[f'AutoRegressionBin'].values, f'AutoRegressionBin-{var}', index, maskNode)
...
def add_past_risk(df, col_pas):
    """Add a shifted risk column computed on xarray objects."""
    ...
    ds['Past_risk'] = shifted
    ds = ds.dropna(dim='sample', subset='Past_risk')
    return ds
